...
#Modified function incorporating inputs 

def adaptive_exp_net_clustered_cog(all_param_dict, input_configs = True):
    """Adaptive exponential integrate-and-fire network with clustered connections."""
    #Cog_input: Enable/disable cog input 
    #Stim_clusters: List of cluster indices for stimulus (None = All exc neurons)
    ...
    ### Define input and weights (Important: Stimulus has to be defined before the equation for timed arrays)
    
    #stim, stim_weights = cog_tasks(all_param_dict, mode ="test", scaling = "e_only")
    #stimulus = b2.TimedArray(stim * b2.amp, dt=b2.defaultclock.dt)
    #Add stimulus to namespace 
    #param_dict_neuron_E['stimulus'] = stimulus 
    #if has_inh:
    #    param_dict_neuron_I['stimulus'] = stimulus  
    ...
    #Setting weights for optional stimulation 
    '''    if cog_input:
        if has_inh:
            E_pop.w_stim = stim_weights[:N_exc]
            I_pop.w_stim = stim_weights[N_exc:]
        else:
            E_pop.w_stim = stim_weights
    else:
        E_pop.w_stim = 0
        if has_inh:
            I_pop.w_stim = 0
    param_dict_neuron_E['stim_weights'] = stim_weights[:N_exc]
    if has_inh:
        param_dict_neuron_I['stim_weights'] = stim_weights[N_exc:]'''
    ...
    #Optional stimulus, with extra choice to only stimulate specific clusters 
    stim_list, weight_list = b2_inputs.cog_tasks(all_param_dict,mode='DM',scaling='e_only')
    config = b2_inputs.get_input_configs([[0],[1]],stim_list,weight_list)
    if input_configs:
        input_op = b2_inputs.create_input_operation(E_pop, config, membership)
